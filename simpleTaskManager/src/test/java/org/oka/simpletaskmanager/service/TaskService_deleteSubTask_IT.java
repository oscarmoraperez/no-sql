...
public class TaskService_deleteSubTask_IT {
    @Autowired
    TaskService service;
    @Autowired
    ...
    public void shouldDeleteSubTask() {
        ...
        // When
        Task task = service.deleteSubTask(persisted, subTask);

        // Then
        assertThat(task.getSubtasks()).isEmpty();
        Task persisted2 = taskRepository.findById(persisted.getId()).orElseThrow();
        assertThat(persisted2.getSubtasks()).isEmpty();
    }
}
